...
public class PollsController {
    ...
    public List<PollCollection> getAllPollsCreatedByUser(@PathVariable("initiatorEmail") String initiatorEmail) {
        LOGGER.info("Entering in rest endpoint to List all polls created by a user by initiatorEmail");
        return pollsService.findByInitiatorEmail(initiatorEmail);
    }

    @ApiOperation(value = "Search polls by its title")
    @RequestMapping(value = "/title/{title}", method = RequestMethod.GET)
    @ResponseStatus(HttpStatus.OK)
    public List<PollCollection> getAllPollsByTitle(@PathVariable("title") String title) {
        LOGGER.info("Entering in rest endpoint to Search polls by its title");
        return pollsService.findByTitle(title);
    }
    ...
    @ResponseStatus(HttpStatus.OK)
    public List<PollCollection> getAllPollsAfterDate(@PathVariable("date") String date) throws ParseException {
        LOGGER.info("Entering in rest endpoint to List all polls created after a certain date");
        return pollsService.findGraterByDate(Long.parseLong(date));
    }
}